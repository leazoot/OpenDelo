-- 00018 · Add the strong auth lockout event type to audit_events.
--
-- Change: widen the event_type CHECK with 'security.strong_auth_locked'.
-- REQ-APPROVAL-005 AC2 requires an audit event when three failed strong auth
-- attempts lock the vault for 60 seconds, and the closed enum had no value for
-- it, so the endpoint had no way to record the lockout.
--
-- Breaking: no. Up only widens the accepted set, so every existing row stays
-- valid and no data is rewritten.
--
-- Rollback: Down rewrites rows of the new type to 'error' before tightening the
-- CHECK, exactly as 00017 does. Deleting them would lose audit rows, and audit
-- rows are never dropped. The rewrite
-- keeps the row, its operation_id and its metadata; only the type becomes
-- coarser. Approved by the user as decision D-14 on 2026-07-30.
--
...
-- indexes. Column list and constraints are copied verbatim from 00017 apart
...
WHERE event_type = 'security.strong_auth_locked';
