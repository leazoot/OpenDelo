-- 00019 · Add the trust memory cleared event type to audit_events.
--
-- Change: widen the event_type CHECK with 'trust.cleared'. REQ-UI-007 AC3 makes
-- clearing a learned authorization a destructive action that must be audited,
-- and the closed enum had no value for it, so neither deletion nor invalidation
-- of a trust memory left any trace in the ledger.
--
...
-- CHECK, exactly as 00017 and 00018 do. Deleting them would lose audit rows, and
-- audit rows are never dropped. The
-- rewrite keeps the row, its operation_id and its metadata; only the type
-- becomes coarser. Approved by the user as decision D-16 on 2026-07-30.
--
...
-- indexes. Column list and constraints are copied verbatim from 00018 apart
...
CREATE TABLE audit_events_rebuilt (
    ...
    event_type             TEXT NOT NULL CHECK (event_type IN (
        ...
        'security.strong_auth_locked', 'trust.cleared'
    )),
...
WHERE event_type = 'trust.cleared';
