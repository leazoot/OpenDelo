-- 00021 · Add the approval expired event type to audit_events.
--
-- Change: widen the event_type CHECK with 'approval.expired'. REQ-CAP-003 AC3
-- requires a ledger entry whenever a request times out at the seam, and the
-- closed enum had no value for it — so an approval that nobody answered could
-- only be closed silently.
--
...
-- CHECK, exactly as 00017, 00018 and 00019 do. Deleting them would lose audit
-- rows, and audit rows are never dropped. The rewrite keeps the row, its
-- operation_id and its metadata; only the type becomes coarser.
--
...
-- indexes. Column list and constraints are copied verbatim from 00019 apart
...
CREATE TABLE audit_events_rebuilt (
    ...
    event_type             TEXT NOT NULL CHECK (event_type IN (
        ...
        'security.strong_auth_locked', 'trust.cleared', 'approval.expired'
    )),
